...
var ovnkCmd = &cobra.Command{
	...
	Long: `Generate or install OVN-Kubernetes support artifacts.

These subcommands are explicit operations and do not read --ovnk-mode.
The --ovnk-mode flag only controls the top-level dpu-sim deployment workflow.`,
}

var ovnkHostAccessCmd = &cobra.Command{
	Use:   "host-access",
	Short: "Create host-cluster access tokens used by DPU-side components",
	Long: `Create host-cluster access tokens used by DPU-side components.

This command always creates host access resources for OVN-Kubernetes and
FRR-K8S. The --ovnk-mode flag only controls the top-level dpu-sim deployment
workflow and does not change the behavior of this subcommand.`,
...
var ovnkValuesCmd = &cobra.Command{
	...
	Long: `Generate OVN-Kubernetes Helm values and DPU-side FRR artifacts.

This command always generates values for the selected cluster(s). The
--ovnk-mode flag only controls the top-level dpu-sim deployment workflow and
does not change the behavior of this subcommand.`,
	RunE: runOVNKValues,
}
